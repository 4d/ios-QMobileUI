...
struct CosmosRating {
    ...
    /**
     ...
     - parameter ratingRemainder: This value is passed from the loop that creates star layers.
       The value starts with the rating value and decremented by 1 when each star is created. For example, suppose we want to display rating of 3.5.
     When the first star is created the ratingRemainder parameter will be 3.5. For the second star it will be 2.5. Third: 1.5. Fourth: 0.5. Fifth: -0.5.
    ...
    /**
     ...
     Rounds a single star's fill level according to the fill mode. "Full" mode returns 0 or 1 by using the standard decimal rounding.
     "Half" mode returns 0, 0.5 or 1 by rounding the decimal to closest of 3 values. "Precise" mode will return the fill level unchanged.
...
@IBDesignable open class CosmosView: UIView {
    ...
    open var cosmosSettings: CosmosSettings = .default {
        didSet {
            update()
        }
    }
    ...
    private func createTextLayer(_ text: String, layers: [CALayer]) -> CALayer {
        let textLayer = CosmosLayerHelper.createTextLayer(text,
                                                          font: cosmosSettings.textFont, color: cosmosSettings.textColor)
        ...
        if RightToLeft.isRightToLeft(self) {
            ...
            CosmosText.position(textLayer, starsSize: starsSize, textMargin: cosmosSettings.textMargin)
        }
    ...
    private func addTextLayer(textLayer: CALayer, layers: [CALayer]) -> [CALayer] {
        ...
        if RightToLeft.isRightToLeft(self) {
            for starLayer in layers {
                starLayer.position.x += textLayer.bounds.width + CGFloat(cosmosSettings.textMargin)
            }
    ...
    private func updateAccessibility() {
        CosmosAccessibility.update(self, rating: rating, text: text, settings: cosmosSettings)
    }
    ...
    open override func accessibilityIncrement() {
        ...
        rating += CosmosAccessibility.accessibilityIncrement(rating, settings: cosmosSettings)
    ...
    open override func accessibilityDecrement() {
        ...
        rating -= CosmosAccessibility.accessibilityDecrement(rating, settings: cosmosSettings)
        didTouchCosmos?(rating)
        didFinishTouchingCosmos?(rating)
    }
    ...
    open override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        if cosmosSettings.passTouchesToSuperview { super.touchesBegan(touches, with: event) }
    ...
    open override func touchesMoved(_ touches: Set<UITouch>, with event: UIEvent?) {
        if cosmosSettings.passTouchesToSuperview { super.touchesMoved(touches, with: event) }
        guard let location = touchLocationFromBeginningOfRating(touches) else { return }
        onDidTouch(location)
    }
    ...
    open override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        if cosmosSettings.passTouchesToSuperview { super.touchesEnded(touches, with: event) }
        didFinishTouchingCosmos?(rating)
    }
    ...
    open override func touchesCancelled(_ touches: Set<UITouch>, with event: UIEvent?) {
        if cosmosSettings.passTouchesToSuperview { super.touchesCancelled(touches, with: event) }
        didFinishTouchingCosmos?(rating)
    }
    ...
    func onDidTouch(_ locationX: CGFloat) {
        let calculatedTouchRating = CosmosTouch.touchRating(locationX, settings: cosmosSettings)

        if cosmosSettings.updateOnTouch {
