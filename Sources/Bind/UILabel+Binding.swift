...
public extension UILabel {
    ...
    @objc dynamic var jsonValues: [String: Any]? {
        ...
        set {
            if let newValue = newValue {
                if let jsonData = try? JSONSerialization.data(withJSONObject: Array(newValue.values), options: []) {
                    self.text = String(data: jsonData, encoding: .utf8)
                } else {
                    self.text = nil
                }
...
extension NSTextAttachment {
    func setImageMaxHeight(_ maxHeight: CGFloat) -> Bool {
        ...
        if ratio >= 1 {
            bounds = CGRect(x: bounds.origin.x, y: bounds.origin.y, width: ratio * maxHeight, height: maxHeight)
        } else {
